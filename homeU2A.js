...
// CODE HERE
const zipCodeIsValid2 = (zipCode) => {
    let validZipCode = false
    deliveryAreaZipCodes.forEach(foo => {
        if (foo === zipCode) {
            validZipCode = true
        }
    })
...
deals[0].title = (updateTitle(deals[0].title))
console.log(deals)
/*
...
//CODE HERE
const updateTitle2 = (desc) => {
    return desc.replace('March', 'April').replace('   ', '')
}
deals[1].desc = (updateTitle2(deals[1].desc))
console.log(deals)